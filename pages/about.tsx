import Head from 'next/head';


const About: React.FC = () => {
  return (
    <div>
      <Head>
        <title>About Watanabe S.</title>
        <meta name="description" content="彼について"/>
      </Head>
      <h2>ABOUT</h2>
      <div className="triangle-bottom" />
      <main>
        <h3>「私について」</h3>
        <p>学歴について、</p>
        <p>幼少から学校に行けず家で勉強していました。</p>
        <p>友達と呼べる人も唯一1人くらいしかなかったです。</p>
        <p>大人になってから国家資格である文部科学省の高認資格を取りました。</p>
        <p>海外の大学への憧れはあるのですが現状は目指していません。</p>
        <p>一度見たものをずっと記憶してしまうという特性を持っています。</p>


        <br />
        <h3>「サイトのアーキテクト」</h3>
        <p>Facebookが元のReactのフレームワークNext.jsによって構築しています。</p>
        <p>ノーコードではなくもはやワンボタンでした。</p>
        <p>ブログは実装が難しくてWordPressなりqiitaなりやってるので宜しくお願いします。</p>
        <p>getStaticPathsもといPropsのコードが長すぎて萎えました。</p>
          <p>ワンボタンで実装して欲しい。</p>
        <p>(実験台だぁ)</p>
        
        <br />
        
        <h3>「設計思想」</h3>
        <h4>スタイルについて</h4>
        <p>カッコよく、美しく、シンプルなモノ。</p>
        <p>元々デザインすることは得意だったためCSSを書くのが好きです。</p>

        <br />

        <h4>コードについて</h4>
        <p>より高速に動く軽量なコードが好きです。</p>
        <p>最小限かつ最大限の役割を果たすことが前提だと思っています。</p>
        <p>あとは意味を付けられない動きや動作はあまり好きではありません。</p>
        <p>そのためコメントアウトを無闇に入れることが嫌いです。</p>
        <p>コード作りの理想は子供に見せて説明しても理解できるものです。</p>
        <p>アイディアを出すことが得意です。</p>
        <br />

        <h4>敬意と感謝</h4>
        <p>子供の頃からAppleやMicrosoftの製品が手元にあったこと、</p>
        <p>この時代と生んでくれた父と母に感謝しています。</p>
      </main>
    </div>
  );
}

export default About;